...
"""
async def get_ssim_band(band_0, band_1, width, height, window_size, pixel_len, c_1, c_2):
    asyncs = []
    for x in range(0, width - width % window_size, window_size):
        for y in range(0, height - height % window_size, window_size):

            asyncs.append(ensure_future(_ssim_tile(x, y, width, window_size, band_0, band_1, pixel_len, c_1, c_2)))

    ssim = sum([await future for future in asyncs])
    return ssim
"""
async def get_ssim_band(band_0, band_1, width, height, window_size, pixel_len, c_1, c_2):
    ssim = 0
...
async def run(image_0, image_1):
    ...
    for i, s in enumerate(image_0.mode):
        ...
        ssim.append(ensure_future(get_ssim_band(band_0, band_1, width, height, window_size, pixel_len, c_1, c_2)))
